<template>
  <div
    class="vdn-drag-source"
    :class="{
      'is-expand': expand,
    }"
    @click="expand = !expand"
  >
    <div class="vdn-drag-source__header">
      <svg
        class="vdn-drag-source__arrow-icon"
        viewBox="0 0 1024 1024"
        fill="currentColor"
        focusable="false"
        aria-hidden="true"
      >
        <path
          d="M512.002047 771.904425c-10.152221 0.518816-20.442588-2.800789-28.202319-10.598382L77.902254 315.937602c-14.548344-14.618952-14.548344-38.318724 0-52.933583 14.544251-14.614859 38.118156-14.614859 52.662407 0l381.437385 418.531212L893.432269 263.004019c14.544251-14.614859 38.125319-14.614859 52.662407 0 14.552437 14.614859 14.552437 38.314631 0 52.933583L540.205389 761.307066C532.451798 769.103636 522.158361 772.424264 512.002047 771.904425z"
        />
      </svg>
      <span>{{ source.title }}</span>
    </div>
    <Draggable
      v-show="expand"
      class="vdn-drag-source__content"
      :list="items"
      :group="{ name: 'vdn-drag-area', pull: 'clone', put: false }"
      :sort="false"
    >
      <DragSourceItem
        v-for="(item, index) in items"
        :item="item"
        :key="index"
        @click.native.stop
      />
    </Draggable>
  </div>
</template>

<script>
import Draggable from 'vuedraggable'

import { getDragSource } from '../mixins/SourceManager'

import DragSourceItem from './DragSourceItem.vue'

export default {
  name: 'vdn-drag-source',
  props: {
    source: {
      type: Object,
      default: () => ({}),
    },
  },
  data() {
    return {
      expand: true,
    }
  },
  computed: {
    items({ source }) {
      return source.items.reduce((list, name) => {
        const ds = getDragSource(name)
        if (ds) {
          list.push(ds)
        }
        return list
      }, [])
    },
  },
  components: {
    Draggable,
    DragSourceItem,
  },
}
</script>
